...
public class EventService implements IEventService {
    ...
    public List<com.dadapp.seniorproject.event.Event> getEventsSearch(String searchString, String ageFilter) {
        ...
        List<Event> matchingEvents = eventRepo.findAll().stream()
                .filter(event -> (event.getTitle().toLowerCase().contains(searchString.toLowerCase())
                        && event.getChildrenAgeRange().toLowerCase().contains(ageFilter.toLowerCase()))
                        || (event.getDescription().toLowerCase().contains(searchString.toLowerCase())
                                && event.getChildrenAgeRange().toLowerCase().contains(ageFilter.toLowerCase()))
                        || (event.getCategory().toLowerCase().contains(searchString.toLowerCase())
                                && event.getChildrenAgeRange().toLowerCase().contains(ageFilter.toLowerCase()))
                        || (event.getAddress().toLowerCase().contains(searchString.toLowerCase())
                                && event.getChildrenAgeRange().toLowerCase().contains(ageFilter.toLowerCase())))
