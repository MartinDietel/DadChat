...
function showDetails(placeResult, marker, status) {
    if (status == google.maps.places.PlacesServiceStatus.OK) {
        ...
        placeInfowindow.setContent(
            '<div><strong>' +
                placeResult.name +
                '</strong><br><div id="markerRatingDiv">' +
                'Rating: ' +
                'None' +
                '</div></div>'
        );
        ...
        success: function (reviewList) {
            ...
            if (reviewList.length <= 0) {
                ...
                reviewHeader.innerHTML =
                    'Reviews &nbsp <button style=\'float: right;\' type="button" class="btn btn-primary" data-toggle="modal" data-target="#addReviewModal" data-placeName="' +
                    placeName +
                    '" data-placeId="' +
                    placeId +
                    '">  Add Review </button>';
                ...
                document.getElementById('ratingText').innerHTML =
                    'Rating: ' +
                    starAverage +
                    ' <span class="fa fa-star checked"></span>'; //add reviews to panel
                document.getElementById('markerRatingDiv').innerHTML =
                    'Rating: ' +
                    starAverage +
                    ' <span class="fa fa-star checked"></span>'; //add reviews to marker info
                ...
                $.each(reviewList, function (index) {
                    ...
                    reviewText.innerHTML =
                        ...
                        ' <h5 class="card-title">' +
                        '<a id="usernameLabel0" class="usernameLinks" href=\'javascript:;\' data-toggle="modal" data-target="#userModal" data-username="' +
                        ...
                        formatDate(reviewList[index].timestamp) +
                        '&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;' +
                        reviewList[index].reviewStars +
...
$(document).ready(function (e) {
        ...
        $.ajax({
            ...
            success: function (response) {
                ...
                reviewText.innerHTML =
                    ...
                    '\n</div>';
                infoPane.appendChild(reviewText);
                $('#newReviewCard' + response.username).insertAfter(
                    '#reviewHeader'
                );
    ...
    function updateReviewStars(placeId) {
        //update reviews stars after adding review
